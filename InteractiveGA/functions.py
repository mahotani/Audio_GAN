...
def random_parent(data, num_parents):
    ...
    for parent in range(num_parents):
        ...
        parents.append(data[parent_index])
...
def get_evaluation_value(vector, solutions, bias):
    ...
    for solution_index in range(len(solutions)):
        evaluations.append(
            euclid_distance(vector, solutions[solution_index]) * 
            (1 - bias[solution_index]))

    minimum = evaluations[0]
    for evaluation_index in range(len(evaluations)):
        if minimum > evaluations[evaluation_index]:
            minimum = evaluations[evaluation_index]
...
def get_result(data, evaluations, num_experiment, best_solution_index, solutions):
    ...
    minimum = evaluations[0]
    nearest_solution_index = 0
    index = 0
    for current_index in range(len(data)):
        if minimum > evaluations[current_index]:
            minimum = evaluations[current_index]
            index = current_index
            nearest_solution_index = get_nearest_solution_index(data[current_index], solutions)
